...
export function AvailabilitySelector({ professionalId, onBooked }) {
  ...
  const handleBooking = async () => {
    ...
    if (!transferConfirmed) {
      setMessage({ type: 'error', text: 'Debes confirmar que realizaste la transferencia.' });
      return;
    }
    ...
    try {
      await api.post('/appointments', {
        ...
        transferProofUrl: transferProofUrl.trim() || null,
        paymentTotal: pricing.total,
      });

      setBookedSlots((prev) => new Set([...prev, selectedSlot]));
      setMessage({ type: 'success', text: 'Pago validado y cita agendada con éxito.' });
      onBooked?.(
        `Cita confirmada para ${new Date(selectedSlot).toLocaleDateString('es-MX', {
          weekday: 'long',
          day: '2-digit',
          month: 'long',
        })} a las ${new Date(selectedSlot).toLocaleTimeString('es-MX', { hour: '2-digit', minute: '2-digit' })}. Total pagado: $${Number(pricing.total).toLocaleString('es-MX')} ${pricing.currency}.`
      );
  ...
  return (
    <div className="card glass-card profile-booking-card" style={{ padding: '1.25rem' }}>
      ...
      {selectedSlot && (
        <div style={{ marginBottom: '1rem', padding: '0.875rem', border: '1px solid var(--outline-variant)', borderRadius: 'var(--radius-lg)', background: 'var(--surface-container-lowest)' }}>
          <p style={{ fontWeight: 700, fontSize: '0.875rem', marginBottom: '0.625rem', color: 'var(--primary)' }}>
            Confirmar transferencia antes de agendar
          </p>
          <input
          ...
          <input
            ...
            style={{ marginBottom: '0.5rem' }}
          />
          <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.8125rem', color: 'var(--on-surface-variant)' }}>
            <input type="checkbox" checked={transferConfirmed} onChange={(e) => setTransferConfirmed(e.target.checked)} style={{ accentColor: 'var(--secondary)' }} />
      ...
      <button
        ...
        disabled={!selectedSlot || isLoading || pricingLoading}
        className="btn btn-primary profile-booking-submit"
        style={{ width: '100%', justifyContent: 'center', opacity: selectedSlot && !isLoading ? 1 : 0.5 }}
      >
        {isLoading ? 'Procesando...' : isAuthenticated ? 'Confirmar pago y agendar cita' : 'Inicia Sesión para Agendar'}
      </button>

      <p style={{ fontSize: '0.75rem', color: 'var(--on-surface-variant)', textAlign: 'center', marginTop: '1rem' }}>
        El pago por transferencia es obligatorio antes de confirmar la cita.
      </p>
    </div>
  );
}
